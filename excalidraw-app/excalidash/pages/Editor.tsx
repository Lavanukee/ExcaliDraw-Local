...
export const Editor: React.FC = () => {
  ...
  const handleMermaidCopy = () => {
    if (!excalidrawAPI.current) return;
    const elements = excalidrawAPI.current.getSceneElementsIncludingDeleted();
    const appState = excalidrawAPI.current.getAppState();
    const selectedElements = appState.selectedElementIds;
    
    let targetElements = elements.filter(el => !el.isDeleted);
    const selectedIds = Object.keys(selectedElements).filter(id => selectedElements[id]);
    if (selectedIds.length > 0) {
      targetElements = targetElements.filter(el => selectedIds.includes(el.id));
    }
  ...
  return (
    <div className="h-screen flex flex-col bg-white dark:bg-neutral-950 overflow-hidden">
      <div className="flex-1 w-full relative h-screen transition-all duration-300">
        {loadError ? (
          ...
          <div className="relative w-full h-full">
            <Excalidraw
              ...
              renderTopRightUI={(_isMobile, appState) => (
                <div className="flex items-center gap-2" style={{ marginRight: '0.5rem' }}>
                  {isSaving && (
                    <div style={{display: 'flex', alignItems: 'center', padding: '0.5rem', fontWeight: 500, color: 'var(--color-primary)'}}>
                      Saving...
                    </div>
                  )}
                  <button
            ...
            <div className="absolute top-4 left-[4rem] z-10" style={{ pointerEvents: 'auto' }}>
              <button
                className="flex items-center gap-2 px-3 h-10 bg-white dark:bg-neutral-800 border border-gray-200 dark:border-neutral-700 rounded-lg shadow-sm hover:bg-gray-50 dark:hover:bg-neutral-700 transition-colors text-sm font-medium text-gray-700 dark:text-gray-300 shadow-[1px_1px_0px_0px_rgba(0,0,0,0.1)]"
                onClick={() => navigate('/')}
            ...
            <div className="absolute right-4 top-1/2 -translate-y-1/2 flex flex-col gap-2 z-10" style={{ pointerEvents: 'none' }}>
              <button
                onClick={() => {
                  excalidrawAPI.current?.updateScene({
                    appState: {
                ...
                className="w-10 h-10 flex items-center justify-center bg-white dark:bg-neutral-800 border-[1px] border-zinc-200 dark:border-neutral-700 rounded-xl shadow-[1px_1px_0px_0px_rgba(0,0,0,0.1)] hover:shadow-[2px_2px_0px_0px_rgba(0,0,0,0.2)] hover:-translate-y-0.5 transition-all text-indigo-600 dark:text-indigo-400"
            ...
            {showMermaidModal && (
              <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
                <div className="bg-white dark:bg-neutral-900 rounded-xl shadow-xl w-full max-w-2xl border border-gray-200 dark:border-neutral-700 flex flex-col overflow-hidden">
                  <div className="flex items-center justify-between p-4 border-b border-gray-200 dark:border-neutral-800 bg-gray-50 dark:bg-neutral-800/50">
                    <h3 className="font-bold text-lg text-slate-900 dark:text-white flex items-center gap-2"><Copy size={18}/> Mermaid Code</h3>
                    <button onClick={() => setShowMermaidModal(false)} className="text-gray-500 hover:text-black dark:hover:text-white text-xl">✕</button>
                  </div>
                  <div className="p-4 overflow-y-auto max-h-[60vh]">
                    <pre className="text-xs bg-gray-900 p-4 rounded-lg overflow-x-auto text-green-400 shadow-inner">
                      <code>{mermaidCode}</code>
                    </pre>
                  </div>
                  <div className="p-4 border-t border-gray-200 dark:border-neutral-800 flex justify-end gap-3 bg-gray-50 dark:bg-neutral-800/50">
                    <button 
                      onClick={() => setShowMermaidModal(false)}
                      className="px-4 py-2 font-medium text-gray-700 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-neutral-700 rounded-lg transition-colors"
                    >
                      Close
                    </button>
                    <button 
                      ...
                      className="px-4 py-2 font-semibold flex items-center gap-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 transition-colors shadow-sm"
                    >
                      Copy Code
                    </button>
                  </div>
